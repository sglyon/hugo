...
> *This tutorial was contributed by [Spencer Lyon](http://spencerlyon.com/).*
...
During this tutorial, I will walk you through the main steps I took to create an example blog available at [http://spencerlyon.com/hugo_gh_blog](http://spencerlyon.com/hugo_gh_blog). The source code for this blog is on [GitHub](https://github.com/sglyon/hugo_gh_blog). Readers are encouraged to download the example repository and follow along.

### Find a Home for Your Files

As our goal is to host a website using GitHub Pages, it is natural for us to host the content of the page in a GitHub repository. Thus, the first step is to either create a new repository on GitHub or create a new directory within an existing repository where the content of the website will live. To do this, I created the repository [sglyon/hugo_gh_blog](https://github.com/sglyon/hugo_gh_blog).
...
The very first step in creating a new Hugo site is to [write the config file](/overview/configuration/). This config file is important for at least two reasons: (1) this is where site-wide settings (like the websites `baseURL`) go, and (2) the config file dictates to some extent how Hugo will generate the website. For the example website I created a file `config.yaml` with the following contents
    ...
    indexes:
      category: "categories"
    baseURL: "http://spencerlyon.com/hugo_gh_blog"
...
> https://github.com/sglyon/hugo_gh_blog for this tutorial, or you *will*
...
After executing these commands and waiting for the GitHub servers to update, the website we just created was live at [http://spencerlyon.com/hugo_gh_blog](http://spencerlyon.com/hugo_gh_blog).
